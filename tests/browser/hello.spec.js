...
test('Pages JS edits update the preview and recover after errors', async ({page}) => {
  ...
  await source.getByRole('combobox', {name:'Update mode'}).selectOption('live');
  ...
  await source.getByRole('combobox', {name:'Update mode'}).selectOption('manual');
  await editor.fill("root.h1('Manual again');");
  await page.waitForTimeout(400);
  await expect(preview.locator('h1')).toHaveText('Recovered');
  await source.getByRole('combobox', {name:'Update mode'}).selectOption('focusout');
  await editor.fill("root.h1('On blur');");
  await expect(preview.locator('h1')).toHaveText('Recovered');
  await source.getByRole('heading').click();
  await expect(preview.locator('h1')).toHaveText('On blur');
  await page.reload();
  await expect(preview.locator('h1')).toHaveText('Hello World');
});
